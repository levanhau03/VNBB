...
xn.get_title_body_script_css = function (s) {
	...
	if(body.indexOf('<meta ') != -1) {
		console.log('Có vấn đề với dữ liệu được tải: body: %s: ', body);
		body = '';
	}
	jtmp.remove();
...
/*
	...
	index.htm
	--------------------------------------------------------------
	<button id="button1" data-modal-url="user-login.htm" data-modal-title="Đăng nhập người dùng" data-modal-arg="xxx" data-modal-callback="login_success_callback" data-modal-size="md"></button>
	<a id="button1" href="user-login.htm" data-modal-title="Đăng nhập người dùng" data-modal-arg="xxx" data-modal-callback="login_success_callback" data-modal-size="md">link</a>
	<a href="user-login.htm" data-modal-title="Đăng nhập người dùng" data-modal-size="md">link</a>
	<script>
	...
	if($action == 'login') {
		if($method == 'GET') {
			...
			// ...
			message(0, 'Đã hạ cánh thành công');
		}
	}
	--------------------------------------------------------------
		...
		<div class="card-body ajax_modal_body">
			<form action="user-login.htm" method="post" id="login_form">
				...
				<div class="form-group">
					<button type="submit" class="btn btn-primary btn-block" data-loading-text="Đang nộp...">Đăng nhập</button>
				</div>
			</form>
		</div>
	</div>	
...
// <button id="button1" class="btn btn-primary" data-modal-url="user-login.htm" data-modal-title="Đăng nhập người dùng" data-modal-arg="xxx" data-modal-callback="login_success_callback" data-modal-size="md">Đăng nhập</button>

$.ajax_modal = function(url, title, size, callback, arg) {
	var jmodal = $.alert('Đang tải...', -1, {size: size});
