...
/********************* xn simulates php functions ************************/

// var xn = window; // browser
...
xn.json_decode = function(s) {
	...
	try {
		...
		//alert('Lỗi định dạng JSON:' + s);
		//window.json_error_string = s;
		return null;
	}
};
...
// Ví dụ: pages('user-list-{page}.htm', 100, 10, 5);
...
xn.image_resize = function(file_base64_data, callback, options) {
	...
	if(thumb_width < 1) return callback(-1, 'thumb image width length is less 1 pix');
	if(xn.substr(file_base64_data, 0, 10) != 'data:image') return callback(-1, 'deformed base64 data');
	// && xn.substr(file_base64_data, 0, 14) != 'data:image/gif' gif\
...
console.log('%c' + 'This is VNBB', 'font-family:Comic Sans MS; font-size:50px; font-weight:bold; background: linear-gradient(#f00, yellow); border-radius: 5px; padding: 20px')